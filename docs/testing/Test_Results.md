...
*(эти кейсы относятся к фронтенду и е2е; не исполнялись в этом прогоне — будут закрываться Playwright/Vitest позднее)*

| ID | Purpose | Scenario | Expected Result | Actual Result | P/F |
|---|---|---|---|---|---|
| TC-TOK-01 | Загрузка списка инструментов | Открыть `/tokens` (prod) | Таблица без ошибок | **Не выполнялся в этом цикле** | NA |
| TC-TOK-02 | Поиск по символу | Ввести `BTC` | Список отфильтрован | Не выполнялся | NA |
| TC-SPR-01…04 | Таблица спредов/сортировка/NaN/порог | Открыть `/spreads` | Соответствие формуле/правилам | Не выполнялись | NA |
| TC-MAT-01…03 | Матрица спредов | Открыть `/matrix` | Корректные значения | Не выполнялись | NA |
| TC-FAV-01…04 | Избранное CRUD/сохранение | Страница `/favorites` | Работает | Не выполнялись | NA |
| TC-ERR-01…03 | Обработка 5xx/429/пустых | Смоделировать ответы | Нотификация/бэк-офф/пустое сост. | Не выполнялись | NA |
| TC-PERF-01…02 | Перфоманс/память | Lighthouse, 10-мин стрим | ≤2.0 сек, прирост <10% | Не выполнялись | NA |
...
## 5. Заключение
Проведённые тесты подтвердили корректность работы модулей backend (FastAPI, WebSocket, REST-парсеры, нормализация данных). Негативные сценарии по WS покрыты. Фронтенд и е2е-сценарии не входили в этот прогон, будут выполнены отдельным циклом. Продукт готов к защите по критериям текущего этапа.
